# 1 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
...
# 1 "<command-line>" 2
# 1 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
# 20 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
# 1 "./config.h" 1
...
# 9 "./config.h" 2
# 21 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 2
...
# 22 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 2
...
# 23 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 2
...
# 24 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 2
...
# 25 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 2
...
{
   return map->size * sizeof (unsigned long);
}
...
sbitmap_resize (sbitmap bmap, unsigned int n_elms, int def)
{
      ...
      if (def)
 {
   ...
   last_bit = bmap->n_bits % ((8 * 8) * 1u);
   if (last_bit)
     bmap->elms[bmap->size - 1]
       |= ~((unsigned long)-1 >> (((8 * 8) * 1u) - last_bit));


   last_bit = n_elms % ((8 * 8) * 1u);
   if (last_bit)
     bmap->elms[size - 1]
       &= (unsigned long)-1 >> (((8 * 8) * 1u) - last_bit);
 }
...
{
  ((void)(!(src->size <= dst->size) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 183, __FUNCTION__), 0 : 0));
...
bitmap_clear_range (sbitmap bmap, unsigned int start, unsigned int count)
{
  ...
  unsigned int start_word = start / ((8 * 8) * 1u);
  unsigned int start_bitno = start % ((8 * 8) * 1u);


  if (start_bitno == 0 && count < ((8 * 8) * 1u))
    {
  ...
  unsigned int end_word = (start + count) / ((8 * 8) * 1u);
  unsigned int end_bitno = (start + count) % ((8 * 8) * 1u);
  ...
  if (start_bitno != 0)
    {
      unsigned int nbits = ((start_word == end_word)
       ...
       : ((8 * 8) * 1u) - start_bitno);
      unsigned long mask = ((unsigned long)1 << nbits) - 1;
      mask <<= start_bitno;
      bmap->elms[start_word] &= ~mask;
      start_word++;
      count -= nbits;
  ...
  if (nwords)
    {
      memset (&bmap->elms[start_word], 0, nwords * sizeof (unsigned long));
      count -= nwords * sizeof (unsigned long) * (8);
      start_word += nwords;
    }
...
{
  ((void)(!(start <= end) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 334, __FUNCTION__), 0 : 0));
  bitmap_check_index (bmap, end);

  unsigned int start_word = start / ((8 * 8) * 1u);
  unsigned int start_bitno = start % ((8 * 8) * 1u);

  unsigned int end_word = end / ((8 * 8) * 1u);
  unsigned int end_bitno = end % ((8 * 8) * 1u);


  if (start_bitno != 0)
    {
      unsigned long high_mask = ~(unsigned long)0;
      if (start_word == end_word && end_bitno + 1 < ((8 * 8) * 1u))
 high_mask = ((unsigned long)1 << (end_bitno + 1)) - 1;

      unsigned long low_mask = ((unsigned long)1 << start_bitno) - 1;
      unsigned long mask = high_mask - low_mask;
 ...
 return true;
  ...
  if (end_bitno + 1 < ((8 * 8) * 1u))
    mask = ((unsigned long)1 << (end_bitno + 1)) - 1;
  return (bmap->elms[start_word] & mask) != 0;
}
# 406 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
unsigned int
...
bitmap_ior_and_compl (sbitmap dst, const_sbitmap a, const_sbitmap b, const_sbitmap c)
{
  ...
  for (i = 0; i < n; i++)
    {
      const unsigned long tmp = *ap++ | (*bp++ & ~*cp++);
      changed |= *dstp ^ tmp;
      *dstp++ = tmp;
    }
...
bitmap_and_compl (sbitmap dst, const_sbitmap a, const_sbitmap b)
{
  ...
  ((void)(!(a->size >= dst_size) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 538, __FUNCTION__), 0 : 0));
  ...
  for (i = 0; i < n; i++)
    {
      ...
      unsigned long wordchanged = *dstp ^ tmp;
      *dstp++ = tmp;
      changed |= wordchanged;
    }
  return changed != 0;
...
bitmap_first_set_bit (const_sbitmap bmap)
{
  ...
  for (bmp_iter_set_init (&(sbi), (bmap), (0), &(n)); bmp_iter_set (&(sbi), &(n)); bmp_iter_next (&(sbi), &(n)))
    return n;
  return -1;
}
...
{
  int i;
  const unsigned long *const ptr = bmap->elms;

  for (i = bmap->size - 1; i >= 0; i--)
    {
      const unsigned long word = ptr[i];

      if (word != 0)
 {
   unsigned int index = (i + 1) * ((8 * 8) * 1u) - 1;
   unsigned long mask
     = (unsigned long) 1 << (((8 * 8) * 1u) - 1);
...
dump_bitmap (FILE *file, const_sbitmap bmap)
{
  ...
  for (i = n = 0; i < set_size && n < total_bits; i++)
    for (j = 0; j < ((8 * 8) * 1u) && n < total_bits; j++, n++)
      {
 if (n != 0 && n % 10 == 0)
   fprintf (file, " ");
...
{
  dump_bitmap (
# 787 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 3 4
              stderr
# 787 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
                    , &ref);
}
...
debug_raw (simple_bitmap_def *ptr)
{
  if (ptr)
    ...
    fprintf (
# 796 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 3 4
            stderr
# 796 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
                  , "<nil>\n");
}
...
{
  dump_bitmap_file (
# 825 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 3 4
                   stderr
# 825 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
                         , bmap);
}
...
{
  dump_bitmap_file (
# 831 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 3 4
                   stderr
# 831 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
                         , &ref);
}
...
debug (simple_bitmap_def *ptr)
{
  if (ptr)
    ...
    fprintf (
# 840 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c" 3 4
            stderr
# 840 "/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c"
                  , "<nil>\n");
}
    ...
    unsigned end)
{
 ...
 break;
  ...
  do { const char *desc_ = "ASSERT_EQ (" "(r1)" ", " "(r2)" ")"; if (((r1)) == ((r2))) ::selftest::pass ((((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 882, __FUNCTION__)))), desc_); else ::selftest::fail ((((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 882, __FUNCTION__)))), desc_); } while (0);
  return r1;
}
...
test_set_range ()
{
  ...
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 0, 0))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 0, 0))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 895, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 895, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 1, 15))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 1, 15))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 896, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 896, __FUNCTION__))), desc_); } while (0);
  bitmap_set_range (s, 15, 1);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 1, 14))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 1, 14))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 898, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 898, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 15, 15))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 15, 15))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 899, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 899, __FUNCTION__))), desc_); } while (0);
  sbitmap_free (s);

  s = sbitmap_alloc (1024);
  bitmap_clear (s);
  bitmap_set_range (s, 512, 1);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 0, 511))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 0, 511))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 905, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 905, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 513, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 513, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 906, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 906, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 512, 512))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 512, 512))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 907, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 907, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 508, 512))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 508, 512))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 908, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 908, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 508, 513))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 508, 513))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 909, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 909, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 508, 511))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 508, 511))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 910, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 910, __FUNCTION__))), desc_); } while (0);

  bitmap_clear (s);
  bitmap_set_range (s, 512, 64);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 0, 511))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 0, 511))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 914, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 914, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p_checking (s, 512 + 64, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 512 + 64, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 915, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 915, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 512, 512))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 512, 512))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 916, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 916, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p_checking (s, 512 + 63, 512 + 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p_checking (s, 512 + 63, 512 + 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 917, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 917, __FUNCTION__))), desc_); } while (0);
  sbitmap_free (s);
}
...
test_bit_in_range ()
{
  ...
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 512, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 512, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 929, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 929, __FUNCTION__))), desc_); } while (0);
  bitmap_set_bit (s, 100);

  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 512, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 512, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 932, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 932, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 0, 99))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 99))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 933, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 933, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 101, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 101, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 934, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 934, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 1, 100))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 1, 100))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 935, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 935, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 64, 100))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 64, 100))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 936, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 936, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 100, 100))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 100, 100))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 937, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 937, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_p (s, 100))" ")"; bool actual_ = ((bitmap_bit_p (s, 100))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 938, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 938, __FUNCTION__))), desc_); } while (0);
  ...
  bitmap_clear (s);
  bitmap_set_bit (s, 63);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 945, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 945, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 1, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 1, 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 946, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 946, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 63, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 63, 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 947, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 947, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_p (s, 63))" ")"; bool actual_ = ((bitmap_bit_p (s, 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 948, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 948, __FUNCTION__))), desc_); } while (0);
  sbitmap_free (s);

  s = sbitmap_alloc (1024);
  bitmap_clear (s);
  bitmap_set_bit (s, 128);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 0, 127))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 127))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 954, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 954, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 129, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 129, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 955, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 955, __FUNCTION__))), desc_); } while (0);

  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 128))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 128))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 957, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 957, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 1, 128))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 1, 128))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 958, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 958, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 128, 255))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 128, 255))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 959, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 959, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 128, 254))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 128, 254))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 960, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 960, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_p (s, 128))" ")"; bool actual_ = ((bitmap_bit_p (s, 128))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 961, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 961, __FUNCTION__))), desc_); } while (0);

  bitmap_clear (s);
  bitmap_set_bit (s, 8);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 8))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 8))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 965, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 965, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 12))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 12))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 966, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 966, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 967, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 967, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 127))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 127))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 968, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 968, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 512))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 512))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 969, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 969, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 8, 8))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 8, 8))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 970, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 970, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_p (s, 8))" ")"; bool actual_ = ((bitmap_bit_p (s, 8))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 971, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 971, __FUNCTION__))), desc_); } while (0);

  bitmap_clear (s);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 0, 0))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 0))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 974, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 974, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 0, 8))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 8))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 975, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 975, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 0, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 63))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 976, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 976, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 1, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 1, 63))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 977, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 977, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 0, 256))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 256))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 978, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 978, __FUNCTION__))), desc_); } while (0);
  ...
  bitmap_set_bit (s, 48);
  bitmap_set_bit (s, 64);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 0, 0))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 0, 0))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 985, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 985, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 1, 16))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 1, 16))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 986, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 986, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 48, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 48, 63))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 987, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 987, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_TRUE (" "(bitmap_bit_in_range_p (s, 64, 64))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 64, 64))); if (actual_) ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 988, __FUNCTION__))), desc_); else ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 988, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 1, 15))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 1, 15))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 989, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 989, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 17, 31))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 17, 31))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 990, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 990, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 49, 63))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 49, 63))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 991, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 991, __FUNCTION__))), desc_); } while (0);
  do { const char *desc_ = "ASSERT_FALSE (" "(bitmap_bit_in_range_p (s, 65, 1023))" ")"; bool actual_ = ((bitmap_bit_in_range_p (s, 65, 1023))); if (actual_) ::selftest::fail (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 992, __FUNCTION__))), desc_); else ::selftest::pass (((::selftest::location ("/home/giulianob/gcc_git_gnu/gcc/gcc/sbitmap.c", 992, __FUNCTION__))), desc_); } while (0);
  sbitmap_free (s);
}
