# 1 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
...
# 1 "<command-line>" 2
# 1 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
# 11 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
# 1 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/mars.h" 1
# 11 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/mars.h"
       
# 57 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/mars.h"
...
# 58 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/mars.h" 2
...
# 12 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c" 2
...
# 13 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c" 2
...
# 14 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c" 2
...
# 31 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/scope.h" 2
...
# 15 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c" 2
...
# 16 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c" 2
...
# 18 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c" 2
...
# 65 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
bool checkParamArgumentEscape(Scope *sc, FuncDeclaration *fdc, Identifier *par, Expression *arg, bool gag)
{
    ...
    for (size_t i = 0; i < er.byvalue.length; i++)
    {
        ...
        else if (v->storage_class & 0x10000LL && p == sc->func)
        {
    ...
    for (size_t i = 0; i < er.byref.length; i++)
    {
        ...
        if ((v->storage_class & (0x200000LL | 0x1000LL)) == 0 && p == sc->func)
        {
    ...
    for (size_t i = 0; i < er.byfunc.length; i++)
    {
        ...
        for (size_t j = 0; j < vars.length; j++)
        {
            ...
            ((void)(!(!v->isDataseg()) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c", 142, __FUNCTION__), 0 : 0));

            Dsymbol *p = v->toParent2();

            v->storage_class &= ~0x1000000000000LL;

            if ((v->storage_class & (0x200000LL | 0x1000LL | 0x80000LL)) && p == sc->func)
            {
...
# 185 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
bool checkAssignEscape(Scope *sc, Expression *e, bool gag)
{
    ...
    for (size_t i = 0; i < er.byvalue.length; i++)
    {
        ...
        if (v->isScope())
        {
            if (va && va->isScope() && va->storage_class & 0x100000000000LL && !(v->storage_class & 0x100000000000LL) &&
                sc->func->setUnsafe())
            {
                if (!gag)
            ...
            if (va && !va->isDataseg() && !va->doNotInferScope)
            {
                ...
                {
                    va->storage_class |= 0x80000LL | 0x2000000000000LL;
                    va->storage_class |= v->storage_class & 0x100000000000LL;
                }
                continue;
            }
            if (sc->func->setUnsafe())
    ...
    for (size_t i = 0; i < er.byref.length; i++)
    {
        ...
        if (va &&
            ((va->enclosesLifetimeOf(v) && !(v->storage_class & 0x20LL)) || va->storage_class & 0x200000LL) &&
            sc->func->setUnsafe())
        {
            if (!gag)
    ...
    for (size_t i = 0; i < er.byfunc.length; i++)
    {
        ...
        for (size_t j = 0; j < vars.length; j++)
        {
            ...
            ((void)(!(!v->isDataseg()) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c", 366, __FUNCTION__), 0 : 0));
...
# 429 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
...
# 481 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
bool checkReturnEscape(Scope *sc, Expression *e, bool gag)
{

    return checkReturnEscapeImpl(sc, e, false, gag);
}
# 498 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
...
static bool checkReturnEscapeImpl(Scope *sc, Expression *e, bool refs, bool gag)
{
    ...
    for (size_t i = 0; i < er.byvalue.length; i++)
    {
        ...
        if ((v->isScope() || (v->storage_class & 0x1000000000000LL)) &&
        ...
        if (v->isScope())
        {
            if (v->storage_class & 0x100000000000LL)
                continue;

            if (sc->_module && sc->_module->isRoot() &&
# 569 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
    ...
    for (size_t i = 0; i < er.byref.length; i++)
    {
        ...
        if ((v->storage_class & (0x200000LL | 0x1000LL)) == 0)
        {
            if (p == sc->func)
            {
            ...
            if (fd && sc->func->flags & 0x10)
            {
                ...
                if (global.params.vsafe)
                    sc->func->storage_class |= 0x100000000000LL;
            }
        }
        ...
        if ( (v->storage_class & (0x200000LL | 0x1000LL)) &&
             !(v->storage_class & (0x100000000000LL | 0x4000LL)))
        {
            if ((sc->func->flags & 0x10) && p == sc->func)
            {
                inferReturn(sc->func, v);
            }
...
# 688 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
static void inferReturn(FuncDeclaration *fd, VarDeclaration *v)
{
    ...
    if (v == fd->vthis)
    {
        ...
        fd->storage_class |= 0x100000000000LL;
        if (tf->ty == Tfunction)
        {
    ...
    else
    {
        ...
        if (tf->ty == Tfunction)
        {
            ...
            for (size_t i = 0; i < dim; i++)
            {
                ...
                if (p->ident == v->ident)
                {
                    p->storageClass |= 0x100000000000LL;
                    break;
                }
            }
        }
...
# 744 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
static void escapeByValue(Expression *e, EscapeByResults *er)
{
    ...
    public:
        ...
        void visit(TupleExp *)
        {
            ((void)(!(0) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c", 812, __FUNCTION__), 0 : 0));
        }
        ...
        void visit(CallExp *e)
        {
            ...
            if (e->arguments && e->arguments->length)
            {
                ...
                for (size_t i = j; i < e->arguments->length; ++i)
                {
                    ...
                    if (i - j < nparams && i >= j)
                    {
                        ...
                        if ((stc & (0x80000LL)) && (stc & 0x100000000000LL))
                            arg->accept(this);
                        else if ((stc & (0x200000LL)) && (stc & 0x100000000000LL))
                            escapeByRef(arg, er);
                    }
                }
            }
            ...
            if (e->e1->op == TOKdotvar && t1->ty == Tfunction)
            {
                ...
                {
                    if (ad->isClassDeclaration() || tf->isscope)
                        dve->e1->accept(this);
                    else if (ad->isStructDeclaration())
                        escapeByRef(dve->e1, er);
                }
                else if (dve->var->storage_class & 0x100000000000LL || tf->isreturn)
                {
                    if (dve->var->storage_class & 0x80000LL)
                        dve->e1->accept(this);
                    else if (dve->var->storage_class & 0x200000LL)
                        escapeByRef(dve->e1, er);
                }
            }
...
# 1023 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
static void escapeByRef(Expression *e, EscapeByResults *er)
{
    ...
    public:
        ...
        void visit(VarExp *e)
        {
            ...
            if (v)
            {
                if (v->storage_class & 0x200000LL && v->storage_class & (0x4000LL | 0x10000000000LL) && v->_init)
                {
                    ...
                    {
                        ((void)(!(ez->exp && ez->exp->op == TOKconstruct) ? fancy_abort ("/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c", 1053, __FUNCTION__), 0 : 0));
        ...
        void visit(CallExp *e)
        {
            ...
            if (tf->isref)
            {
                if (e->arguments && e->arguments->length)
                {
                    ...
                    for (size_t i = j; i < e->arguments->length; ++i)
                    {
                        ...
                        if (i - j < nparams && i >= j)
                        {
                            Parameter *p = tf->parameterList[i - j];
                            const StorageClass stc = tf->parameterStorageClass(p);
                            if ((stc & (0x1000LL | 0x200000LL)) && (stc & 0x100000000000LL))
                                arg->accept(this);
                            else if ((stc & 0x80000LL) && (stc & 0x100000000000LL))
                            {
                ...
                {
                    DotVarExp *dve = (DotVarExp *)e->e1;
                    if (dve->var->storage_class & 0x100000000000LL || tf->isreturn)
                    {
                        if ((dve->var->storage_class & 0x80000LL) || tf->isscope)
                            escapeByValue(dve->e1, er);
                        else if ((dve->var->storage_class & 0x200000LL) || tf->isref)
                            dve->e1->accept(this);
                    }
...
# 1211 "/home/giulianob/gcc_git_gnu/gcc/gcc/d/dmd/escape.c"
